...
function loadCities(contacts) {
    ...
    let content = '<option value="0">-- Select a city --</option>';
    let cityCollection = [];
    for (let i = 0; i < contacts.length; i++) {
        if (cityCollection.includes(contacts[i].address.city)) {
            continue;
        } else {
            content += `<option value="${contacts[i].address.city}">${contacts[i].address.city}</option>`
            cityCollection.push(contacts[i].address.city)
        }
    }
    filterOptions.innerHTML = content;
}
...
function main() {
    const deleteBtn = document.querySelector('main');
    const selection = document.querySelector('#filterOptions');
    selection.addEventListener("change", filterHandler)
